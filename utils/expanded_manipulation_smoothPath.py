...
class ManipulationModelling:
    ...
    def grasp_top_box_atGivenHeight(self, time, obj, gripper, height, grasp_direction='xz'):
        """
        grasp a box from the top at a specific distance from the top plane (instead of the center)  
        """
        if grasp_direction == 'xz':
            align = [ry.FS.scalarProductXY, ry.FS.scalarProductXZ, ry.FS.scalarProductYZ]
        elif grasp_direction == 'yz':
            ...
            raise Exception('pickDirection not defined:', grasp_direction)
        # position: height above lower plane of objectheight
        # self.komo.addObjective([time-.2,time], ry.FS.positionRel, [gripper, obj], ry.OT.eq, [1e1], [0, 0, height])
        self.komo.addObjective([time-.2,time], ry.FS.positionDiff, [gripper, obj], ry.OT.eq, [1e1], [0, 0, -height])
    ...
    def setGripperForPulling(self, gripper: str, width: float):
        """
        Sets the initial gripper position for pulling the object: 
        1. define a plane parallel to the outer surface of the object but distance to the object's center is width/2
        set pull position: allign gripper with box center of top face
        """
        
        # set gripper width
        # self.komo.addObjective([1.], ry.FS.qItself, [gripper], ry.OT.eq, [1e1], [width/2])

        # introduce a helper frames for the 2 contact planes of the gripper
        self.add_helper_frame(ry.JT.transXYPhi, gripper, 'pull_plane_left', gripper)
        self.add_helper_frame(ry.JT.transXYPhi, gripper, 'pull_plane_right', gripper)
        self.komo.addObjective([1.], ry.FS.position, ['pull_plane_left'], ry.OT.eq, [1e1], [0, 0, width/2])
        self.komo.addObjective([1.], ry.FS.position, ['pull_plane_right'], ry.OT.eq, [1e1], [0, 0, -width/2])

        # contact plane of the gripper
        self.add_helper_frame(ry.JT.transXYPhi, gripper, 'pull_contact_plane', gripper)
        self.komo.addObjective([1.], ry.FS.position, ['pull_contact_plane'], ry.OT.eq, [1e1], [0, 0, 0])

        # align gripper with the object
        self.komo.addObjective([1.], ry.FS.positionDiff, ['pull_plane_left', 'pull_plane_right'], ry.OT.eq, [1e1])
        self.komo.addObjective([1.], ry.FS.positionDiff, ['pull_plane_left', 'pull_contact_plane'], ry.OT.eq, [1e1])
        self.komo.addObjective([1.], ry.FS.positionDiff, ['pull_plane_right', 'pull_contact_plane'], ry.OT.eq, [1e1])
    ...
    def pull(self, times, obj, gripper, table):
        ...
        # constrain gripper and object to be aligned??
...
# Eckhard's code
    ...
    def follow_path_on_plane(self, path: list, plane: str="z", moving_frame: str="l_gripper", soft=False, initialization=None):
        """
        ...
        plane_normal = plane_pos/np.linalg.norm(plane_pos)
        
        phases = len(path)-1
        self.komo = ry.KOMO()
        self.komo.setConfig(self.C, True)
        ...
        self.komo.addObjective([1, phases], ry.FS.vectorZ, [moving_frame], ry.OT.eq, [1e1], plane_normal)
        self.komo.addObjective([1, phases], ry.FS.position, [moving_frame], ry.OT.eq, plane_normal*1e1, plane_pos)

        res = np.array([1, 1, 1]) - plane_normal
        for i in range(1, len(path)):
            if not soft:
                self.komo.addObjective([i], ry.FS.position, [moving_frame], ry.OT.eq, res, threeD_path[i])
            else:
                self.komo.addObjective([i], ry.FS.position, [moving_frame], ry.OT.sos, 1e-1*res, threeD_path[i])

        if soft:
            self.komo.addObjective([], ry.FS.accumulatedCollisions, [], ry.OT.eq, 1e1)
            print('we are here')
            # self.komo.addObjective([], ry.FS.negDistance, ['box', 'longBlock'], ry.OT.ineq, 1e2)
            i = len(path)-1
            self.komo.addObjective([i], ry.FS.position, [moving_frame], ry.OT.eq, res, threeD_path[i])

        if initialization is not None:
            self.komo.initWithPath(initialization)
            # self.komo.view(True)
